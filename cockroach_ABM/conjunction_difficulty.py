...
n_repeats = 500
target_proportions = np.zeros((n_difficulty_range, n_repeats))  
time_constants = np.zeros((n_difficulty_range, n_repeats))  


for idx, difficulty in tqdm(enumerate(difficulty_range), total=n_difficulty_range):
    sizeLQ = difficulty
    sizeHQ = 1.0
    lightLQ = difficulty
    lightHQ = 1.0
    ...
    s = np.array([N, config[0]*N])
    theta = np.array([theta_base, config[2]*theta_base])

    for k in range(n_repeats):
        agents = [Cockroach_agent_basic(nD, t_max) for _ in range(N)]
...
plt.xticks(np.arange(len(difficulty_range)) + 0.5, difficulty_range, fontsize=14)
