...
fig, axes = plt.subplots(len(sizeLQ), 1, figsize=(9,13), sharex = True)
for i, ax in enumerate(axes):
    ...
    # Project ns-dimensional trajectory to (x, y=sum distractors)
    x_traj = sol.y[0]
    y_traj = sol.y[1]

    if i == 1:
        ax.plot(x_traj, y_traj, color='tab:blue', lw=4, label='simulation', zorder = 4, linestyle = '--')
    else:
...
axes[len(sizeLQ)-1].set_xticks([0, 0.5, 1],[0, 0.5, 1])
# axes[3].set_xlabel('Proportion under \nlarge shelter')
plt.show()
...
### INSET FIGURE
DX, DY, sol = get_ports(8, 8)

fig, ax = plt.subplots(figsize=(1.5,1.5))

ax.fill_between([0, N], [N, 0], color='#F3F4F6', zorder=0)
...
Um = np.ma.masked_invalid(DX)
Vm = np.ma.masked_invalid(DY)
ax.streamplot(grid, grid, Um, Vm, density=4, arrowsize=1.5, minlength=0.05, color= 'grey')
